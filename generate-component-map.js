...
async function printGraph(options) {
  ...
  const dependenciesFilter = (dependency) => dependency.startsWith('vl-') && dependency !== 'vl-ui-core' && dependency !== 'vl-ui-util' && !options.exclusions.includes(dependency);
  const dependencyLogFunction = (componentName, dependency, version) => {
    return `"${componentName}" -> "${dependency}" [style=solid]`;
  };
  const devDependencyLogFunction = (componentName, dependency, version) => {
    return `"${componentName}" -> "${dependency}" [style=dotted]`;
  };
...
printGraph(options()).catch((error) => console.error(error));
